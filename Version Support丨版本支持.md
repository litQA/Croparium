# New Version Progress丨新版本开发进度
## Croparium v5.0
### Progress
█████████░░░░░░░░░ 47.83%

### Tasks

#### Main Mod Compatibility

* [x] Main Mod \- Forge 1\.18\.2

* [x] Main Mod \- Forge 1\.19\.4

* [x] Main Mod \- Forge 1\.20\.1

* [x] Main Mod \- NeoForge 1\.21\.1

* [ ] Main Mod \- NeoForge 1\.21\.4

* [x] Main Mod \- NeoForge 1\.21\.8

* [ ] Main Mod \- NeoForge 26\.1\.2

* [x] Main Mod \- Fabric 1\.21\.8

* [ ] Main Mod \- Fabric 26\.1\.2

#### Expansion Mod Compatibility

* [x] Expansion Mod \- Aether

* [x] Expansion Mod \- Arphex

* [x] Expansion Mod \- Composite Material

* [x] Expansion Mod \- Confluence Otherworld

* [ ] Expansion Mod \- Create

* [ ] Expansion Mod \- L\_Enders Cataclysm

* [ ] Expansion Mod \- Mekanism

* [ ] Expansion Mod \- Twilight Forest

* [x] Expansion Mod \- BOP Legacy Reborn

* [ ] Expansion Mod \- Thermal \(Newly Supported\)


# Version Support丨版本支持
|Symbol/符号|Meaning|含义|
|:----------|:--------|:--------|
|**RR‑AS**|Regular Release- Active Support|常规版・活跃维护期|
|**LTS‑AS**|Long-Term Support- Active Support|LTS长期版・活跃维护期|
|**LTS‑ES**|Long-Term Support- Extended Support|LTS长期版・扩展维护期|
|**EOL**|End of Life|停止全部更新|
|**DEV**|Developing- No stable version|开发中，无稳定版|

## Minrcraft Forge
|Minrcraft Version|1.18.2|1.19.2|1.19.4|1.20.1|
|:----------:|:--------:|:--------:|:--------:|:--------:|
|Min Forge Version|40.3.0|43.5.0|45.4.0|47.4.0|
|Status|LTS‑AS|LTS‑ES|LTS‑AS|LTS‑AS|
|Latest Stable Version|4.8.3|4.8.3|4.8.3|4.8.0|

## NeoForge (20.4-21.3)
|Minrcraft Version|1.20.4|1.20.6|1.21|1.21.1|1.21.2|1.21.3|
|:----------:|:--------:|:--------:|:--------:|:--------:|:--------:|:--------:|
|Min NeoForge Version|20.4.251|20.6.139|21.0.167|21.1.233|"21.2.1-beta"|21.3.95|
|Status|EOL|EOL|EOL|LTS‑AS|EOL|EOL|
|Latest Stable Version|4.5.5|4.5.5|3.4.0|4.8.0|3.4.0|3.4.0|

## NeoForge (21.4-21.8)
|Minrcraft Version|1.21.4|1.21.5|1.21.6|1.21.7|1.21.8|
|:----------:|:--------:|:--------:|:--------:|:--------:|:--------:|
|Min NeoForge Version|21.4.155|21.5.75|"21.6.20-beta"|"21.7.25-beta"|21.8.53|
|Status|RR‑AS|EOL|EOL|EOL|RR‑AS|
|Latest Stable Version|4.8.3|3.3.0|3.3.1|4.4.0|4.8.0|

## NeoForge (26.x)
|Minrcraft Version|26.1|26.1.1|26.1.2|
|:----------:|:--------:|:--------:|:--------:|
|Min NeoForge Version|"26.1.0.19-beta"|"26.1.1.15-beta"|26.1.2.71|
|Status|DEV|DEV|DEV|
|Latest Stable Version|-|-|-|

## Fabric
|Minrcraft Version|1.21.6|1.21.7|1.21.8|26.1|26.1.1|26.1.2|
|:----------:|:--------:|:--------:|:--------:|:--------:|:--------:|:--------:|
|Min Fabric API Version|0.128.2|0.129.0|0.136.1|0.145.1|0.145.4|0.150.0|
|Status|DEV-Alpha|DEV-Alpha|DEV-Alpha|DEV|DEV|DEV|DEV|
|Latest Stable Version|-|-|-|-|-|-|

